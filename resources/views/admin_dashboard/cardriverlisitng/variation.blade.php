...
@section('content')
    ...
    <div class="container-fluid">
                ...
                <div class="card tabsBox" id="tabs_id">
                    <ul class="nav nested nav-pills mb-3" id="pills-tab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a href="{{ route('edit.product', session()->get('var_product_id') ?? $id) }}">
                    ...
                    <div class="container">
                        <form id="variationform" action="{{ route('add.product_variation') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="accordion mb-3" id="accordionPanelsStayOpenExample">
                                ...
                                @foreach ($product->product_variations as $key => $item)
                                        ...
                                        // array_push($prod_attributes,$item->attribute_id);
                                        @endphp
                                    <div class="accordion-item">
                                        <h2 class="accordion-header">
                                            @if (!empty($product->product_variants))
                                                ...
                                                {{-- if variation has not attribute added it escape it --}}
                                                {{-- @if (count($product_attribute->product_attributes) == 0)
                                                    @continue
                                                @endif --}}
                                                    ...
                                                    <div class="select-dropdown selectDropdown">
                                                        <select class="form-control edit_attribute_e_{{ $key }} detect_attribute_similarties" name="attributes[]" onchange="return edit_attribute('{{ $key }}')">
                                                            @foreach ($product_attribute->product_attributes as $attr)
                                                                @php
                                                                    array_push($prod_attributes,$attr);
                                                                @endphp
                                                                <option value="{{ $attr->attribute_id }}"{{ in_array($attr->attribute_id, $attribute_ids_array) ? 'selected' : '' }}>
                                                                        {{ $attr->attribute }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        ...
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput10">Regular
                                                                    price.*</label>
                                                                <input type="text" id="regular" class="form-control regular_price_{{ $key }} salePrice"
                                                                    name="regular_price[]"
                                                                    value="{{ $item->regular_price }}"
                                                                    placeholder="Regular price" onchange="regularPrice('{{ $key }}','')" id="regular_price_{{ $key }}">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                ...
                                                                <input type="number" class="form-control sale_price_{{ $key }} salePrice"
                                                                    placeholder="Sale price" id="sale" name="sale_price[]"
                                                                    value="{{ $item->sale_price }}" onchange="salePriceOnKeyChange('{{ $key }}','')" onkeyup="salePriceOnKeyUp('{{ $key }}','')" id="sale_price_{{ $key }}">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                ...
                                                                <input type="number" class="form-control salePrice"
                                                                    placeholder="Quantity" id="qty" name="quantity[]"
                                                                    value="{{ $item->quantity }}">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3">
                                                        ...
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                ...
                                                                <input type="date" name="start_date[]"
                                                                    class="form-control sale_start sale_start_{{ $key }}"
                                                                    placeholder="Sale start date"
                                                                    value="{{ !empty($item->start_date) ? date_format(date_create($item->start_date), 'Y-m-d') : null }}" onchange="saleStartOnChange('{{ $key }}','')">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                ...
                                                                <input type="date" name="end_date[]"
                                                                    class="form-control sale_end sale_end_{{ $key }}"
                                                                    placeholder="Sale end date"
                                                                    value="{{ !empty($item->end_date) ? date_format(date_create($item->end_date), 'Y-m-d') : null }}" onchange="saleEndOnChange('{{ $key }}','')">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="row mt-4 mb-4">
        ...
        <h2 class="accordion-header">
                 ...
                 @php
//    $a=implode("\n", range(0, count($item)-1));
// //   settype($a, "integer"); // $a is now integer
//  echo (number)$a;
$prod_attribute_seesion =[];
$optionValue=[];

@endphp
            {{-- {{ dd($product_attributes) }} --}}
        @if (!empty($product_attributes))
        @foreach ($product_attributes as $keysa=>$product_attribute)
        <div class="select-dropdown selectDropdown">
            <select class="form-control edit_attribute_{{ $keysa }} checkedDefault detect_attribute_similarties" name="attributes[]" onchange="return edit_attribute('{{ $keysa }}')">



                @foreach ($product_attribute->product_additional_attributes as $attr)
                @php
                    array_push($prod_attribute_seesion,$attr);
                @endphp
                <option value="{{ $attr->attribute_id }}" @if(gettype($item)=='string' ) {{ $attr->attribute_id==$item?"selected":'' }} @else @if($keysa<=count($item)-1) {{ $attr->attribute_id==$item[$keysa]?"selected":'' }} @endif @endif>
                    {{ $attr->attribute }}</option>

                @php

                array_push($optionValue,$attr->attribute_id);
                @endphp
                @endforeach
        ...
        {{-- update work 26 --}}
        <p id="optionCount" hidden>{{ gettype($item)=='string'?$item:count($item) }}</p>
        @endforeach
        {{-- <p id="optionCount" hidden>{{ count($optionValue); }}</p> --}}
        @endif
                                            <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                                    ...
                                                    <div class="row mt-4 mb-4">
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput10">Regular
                                                                    price.*</label>
                                                                <input type="text" class="form-control salePrice regular_price_e_{{ $key }}"
                                                                    name="regular_price[]" placeholder="Regular price" value="{{old('regular_price[]')}}" onchange="regularPrice('{{ $key }}','e_')" id="regular_price_e_{{ $key }}">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput10">Sale price.* <span
                                                                        class="schedule">Schedule</span></label>
                                                                <input type="text" class="form-control sale_price_e_{{ $key }} salePrice"
                                                                    placeholder="Sale price" id="sale" name="sale_price[]" value="{{old('sale_price[]')}}" onchange="salePriceOnKeyChange('{{ $key }}','e_')" onkeyup="salePriceOnKeyUp('{{ $key }}','e_')" id="sale_price_e_{{ $key }}">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput10">Quantity.*</label>
                                                                <input type="text" id="qty" class="form-control salePrice"
                                                        ...
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput10">Sale start
                                                                    date.*</label>
                                                                <input type="date" name="start_date[]"
                                                                    class="form-control sale_start sale_start_e_{{ $key }}"
                                                                    placeholder="Sale start date" value="{{old('start_date[]')}}" onchange="saleStartOnChange('{{ $key }}','e_')">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput10">Sale end
                                                                    date.*</label>
                                                                <input type="date" name="end_date[]"
                                                                    class="form-control sale_end sale_end_e_{{ $key }}"
                                                                    placeholder="Sale end date" value="{{old('end_date[]')}}" onchange="saleEndOnChange('{{ $key }}','e_')">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="row mt-4 mb-4">
                                                        <div class="col-md-6">
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput10">Weight.*</label>
                                                                <input type="text" name="weight[]"
                                                                    class="form-control" placeholder="Weight" value="{{old('weight[]')}}">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-2">
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput10">Length.*</label>
                                                                <input type="text" name="length[]"
                                                                    class="form-control" placeholder="Length" value="{{old('length[]')}}">
                                                            </div>
                                                        </div>
                                                        <div class="col-md-2">
                                                            <div class="form-group">
                                                        ...
                                                        <div class="col-md-2">
                                                            <div class="form-group">
                                                                ...
                                                                <input type="text" name="height[]"
                                                                    class="form-control" placeholder="Height" value="{{old('height[]')}}">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="row mt-4 mb-4">
                                                        <div class="col-md-3">
                                                            ...
                                                            <div class="select-dropdown">
                                                                <select class="form-control" name="stock[]">
                                                                    ...
                                                                    <option value="2" {{ old('stock[]') == 2 ? 'selected' : '' }}>Out stock</option>
                                                                </select>
                                                            </div>
                                                        </div>
                                                    </div>
    ...
    @push('scripts')
        ...
        {{-- <script>
            ...
            function edit_attribute(key) {
                ...
                let newValue = [];
                option = [];
                optionCount=$('#optionCount').text();
console.log(optionCount)
                $.each($(val), function(indexInArray, valueOfElement) {
                    option.push($(this).val());
                });
         $.each($(".checkedDefault"), function (indexInArray, valueOfElement) {
                    attr_total_arr.push(indexInArray);
                })
//                 for($a=0; $a<attr_total_arr.length; $a++){
// if($a!=key){
                $.each($('.edit_attribute_' + 0), function (indexInArray, valueOfElement) {
...
console.log(option,'a')
  ...
  console.log(disable = JSON.stringify(valueOfElement.option) === JSON.stringify(option))
            ...
            for (i=0; i < allValues.length; i += 3) {
            let tempArray;

            tempArray = allValues.slice(i, i + 3);
  console.log(tempArray)
            if (JSON.stringify(tempArray) === JSON.stringify(option)) {
                console.log(JSON.stringify(tempArray),JSON.stringify(option))
                toastr.error('variation already exist');
                console.log(tempArray,'d',)
            }
            }
                var arr = {
                    'key': key,
        ...
        <script>
            ...
            $('.checkedDefault').map(function(_, el) {
            ...
            function edit_attribute(key) {
                ...
                option = [];
                optionCount = $('#optionCount').text();
                // console.log($('.edit_attribute_'));

                $('.checkedDefault').map(function(_, el) {
                attr_total_arr.push($(el).val());
                return $(el).val();
                }).get();
//                 const   arrds=[];
//    for ( i = 1; i <=54; i+=optionCount) {

//     arrds.push(attr_total_arr.slice(i,i+optionCount));
// }
// console.log(arrds)
// // console.log(newValue)
    ...
    $.ajax({
        type: "GET",
        url: '{{ route("optionCheck") }}',
        "dataSrc": "",
        data: {'optionCount': optionCount,'attr_total_arr': attr_total_arr,
        },
        ...
        success: function(response) {
            ...
            if(response.error){
                ...
                // $.each($(add_variation), function(indexInArray, valueOfElement) {
                //     console.log(valueOfElement,option)
                //     if (valueOfElement.key == key) {
                //         delete add_variation.key;
                //     } else if (valueOfElement.key != key) {
                //         console.log(disable = JSON.stringify(valueOfElement.option) === JSON.stringify(option),'as')
                //         if (disable) {
                //             toastr.error('variation already exist');
                //             $("#saveVariations").attr('disabled', 'disabled');
                //         } else {
                ...
                // for (i = 0; i < attr_total_arr.length; i += optionCount) {
                //     let tempArray;
                //     tempArray = attr_total_arr.slice(i, i + optionCount);
                //     newValue.push(tempArray);
                // }
                // console.log(newValue.slice(0, 2));
        ...
        function salePriceOnKeyUp(key,edit=null) { 
            
            var regular_price = $(`.regular_price_${edit}${key}`).val();
            let sale_price = $(`.sale_price_${edit}${key}`).val();
            if (parseInt(sale_price) === null || parseInt(sale_price) == 0) {
                $(`.sale_price_${edit}${key}`).val("");
            }

            if(parseInt(sale_price) >= parseInt(regular_price)){
                toastr.error('Sorry, sale price must be less than regular price.');
                $(`.sale_price_${edit}${key}`).val("");
                $(`.sale_price_${edit}${key}`).focus();
            }
            checkEachRowDateValidation();
        }

        function salePriceOnKeyChange(key,edit=null) { 
            var regular_price = $(`.regular_price_${edit}${key}`).val();
            let sale_price = $(`.sale_price_${edit}${key}`).val();
        ...
        function saleEndOnChange(key,edit=null) { 
            var sale_start = $(`.sale_start_${edit}${key}`).val();
            var sale_end = $(`.sale_end_${edit}${key}`).val();
            
            if(sale_start == '' && sale_end == ''){
                $(`.sale_start_${edit}${key}`).removeClass('is-error');
                $(`.sale_end_${edit}${key}`).removeClass('is-error');
                $("#saveVariations").attr('disabled', false);
                $("#saveVariations").removeClass("not-allowed");
                checkEachRowDateValidation();
                return false;
            }
            ...
            if(sale_start != '' && sale_end != ''){
                ...
                if(new Date(sale_start) >= new Date(sale_end)){
                    $(`.sale_end_${edit}${key}`).val("");
                    $(`.sale_end_${edit}${key}`).focus();
                    $(`.sale_end_${edit}${key}`).addClass('is-error');
                    $(`.sale_start_${edit}${key}`).addClass('is-error');
                    $(`.sale_start_${edit}${key}`).focus();
                    $("#saveVariations").attr('disabled', true);
                    $("#saveVariations").addClass("not-allowed");
        ...
        function regularPrice(key,edit=null) { 
                // alert(key);
        }

        function saleStartOnChange(key,edit=null) { 

            var sale_start = $(`.sale_start_${edit}${key}`).val();
            var sale_end = $(`.sale_end_${edit}${key}`).val();
            console.log(new Date(sale_start));
         ...
         function checkEachRowDateValidation(){
            let count = 0;
            $.each($(".sale_start"), function (indexInArray, valueOfElement) { 
                if($(this).hasClass('is-error') || $(".sale_end").hasClass('is-error')){
                    count += 1;
